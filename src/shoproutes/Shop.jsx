...
const Shop = () => {
  ...
  const [shopData, setShopData] = useState(
    localStorage.getItem("shopFilter") || "All"
  );
    ...
    <div className="pt-24 max-w-[1400px] mx-auto">
      <div>
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-semibold text-green-600">Shop Items</h1>
        ...
        <div className="flex justify-center pt-16">
          <div className="grid grid-cols-4 gap-5">
            {filterShop?.length > 0 ? (
              [...filterShop]
                ?.reverse()
                .map((item) => <Shopcard key={item._id} item={item} />)
            ) : (
              <p className="text-xl text-red-600 font-semibold py-12">
